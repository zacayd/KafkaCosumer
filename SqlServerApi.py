...
class SqlServerApi:
    ...
    def SourceToTargetLoad(self,df):
            ...
            try:
                ...
                for index, row in dfDistinct.iterrows():
                    ...
                    try:
                        log.info(statement)
                        self.cursor.execute("delete from TI.MNG_OCTOPAI_LINEAGE_SOURCE_TO_TARGET where CONNECTION_ID= ? and CONTAINER_OBJECT_PATH=? and  CONTROL_FLOW_PATH=? "
    ...
    def DiscoveryTableLoad(self, df):
        ...
        try:
            self.cnxn = pyodbc.connect(
                'DRIVER={ODBC Driver 18 for SQL Server};SERVER=' + self.server + ';DATABASE=' + 'oumdb' + ';UID=' + self.usr + \
                ';PWD=' + self.pwd + ';TrustServerCertificate=Yes;')
            cursor = self.cnxn.cursor()

            cursor.execute("SELECT * FROM TI.MNG_EXTRACTOR_QUERIES where TOOL_NAME=?","DATABRICKS_POST")
                ...
                for index, row in dfDistinct.iterrows():
                    ...
                    if tableName!="TI.MNG_SQLS_AUTO_COMPLETE":
                        try:
                            ...
                            self.cursor.execute(stmt,ConnectionID,ContainerObjectPath,ControlFlowPath,LayerName)
                            self.cursor.commit()
                        except Exception as e:
                            log.error(e)
                    try:

                        self.cursor.execute(queryText,ConnectionID,ContainerObjectPath,ControlFlowPath,LayerName)
                        # log.info(queryText)
                        self.cursor.commit()
                        cursor.commit()
                    except Exception as e:
    ...
    def InsertStatmentMngLineageUIDatabase(self,tableName,dictCols):
        ...
        try:
            ...
            self.cursor.execute(insertTxt,dictCols[0],
                                         dictCols[1],
                                         dictCols[2],
                                         dictCols[3],
                                         dictCols[4],
    ...
    def LoadLineageObject(self,df):
        ...
        try:
            ...
            for i, row in dfLineageObject.iterrows():
                ...
                self.cursor.execute("SELECT ObjectGUID FROM TI.LINEAGEOBJECT where ObjectGUID=?",row['ObjectGUID'])
            ...
            self.cursor.execute("SELECT ObjectGUID FROM TI.LINEAGEOBJECT where ObjectGUID=?",
    ...
    def dfMngLineageUiDatabase(self, dfLineageObject, tableName):
        ...
        try:
            ...
            for i, row in dfMNG_LINEAGE_UI_DATABASE.iterrows():
                ...
                self.cursor.execute(f"SELECT *  FROM {self.schema }.{tableName} where  CONNECTION_ID=? and DATABASE_NAME=? and "
    ...
    def dfPopAutoComplete(self, dfLineageObject, tableName):
        try:
            ...
            self.cursor.execute("SELECT CONNECTION_ID FROM TI.MNG_SQLS_AUTO_COMPLETE where TABLE_NAME=? and CONNECTION_ID=?",
